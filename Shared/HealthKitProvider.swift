...
class HealthKitProvider: ObservableObject {
    ...
    func fetchActivitySummary() async throws -> ActivitySummaryModel {
        let calendar = Calendar.current
        let now = Date()
        let startOfDay = calendar.startOfDay(for: now)
        let endOfDay = calendar.date(byAdding: .day, value: 1, to: startOfDay) ?? now
        ...
        async let basalEnergy = fetchBasalEnergy(from: startOfDay, to: endOfDay)
        async let exerciseMinutes = fetchExerciseMinutes(from: startOfDay, to: endOfDay)
        async let standHours = fetchStandHours(from: startOfDay, to: endOfDay)
        
        let (stepsCount, distanceTotal, activeEnergyTotal, basalEnergyTotal, exerciseTime, standTime) = try await (
            steps, distance, activeEnergy, basalEnergy, exerciseMinutes, standHours
        )
        
        // Calculate active minutes (simplified - could be more sophisticated)
        let activeMinutes = Int(exerciseTime) + (Int(stepsCount) / 100) // Rough estimate
        
        return ActivitySummaryModel(
            date: now,
            steps: Int(stepsCount),
            distance: distanceTotal,
            activeEnergy: activeEnergyTotal,
            basalEnergy: basalEnergyTotal,
            activeMinutes: activeMinutes,
            standHours: Int(standTime),
            exerciseMinutes: Int(exerciseTime),
            stepGoal: 10000, // Default goals - can be made configurable
            activeEnergyGoal: 400.0,
            exerciseGoal: 30,
            standGoal: 12
        )
    }
    
    // MARK: - Health Metrics Data
    func fetchHealthMetrics() async throws -> HealthMetricsModel {
        let now = Date()
        
        // Fetch health metrics in parallel
        async let heartRate = fetchLatestHeartRate()
        async let heartRateVariability = fetchLatestHeartRateVariability()
        async let oxygenSaturation = fetchLatestOxygenSaturation()
        async let bodyTemperature = fetchLatestBodyTemperature()
        async let bloodPressure = fetchLatestBloodPressure()
        
        let (hr, hrv, spo2, temp, bp) = await (
            heartRate, heartRateVariability, oxygenSaturation, bodyTemperature, bloodPressure
        )
