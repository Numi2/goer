/*
Abstract:
Widget components using Apple's native glass effects for proper transparency
and morphing following Apple's SwiftUI design principles.
*/

import SwiftUI

// MARK: - Apple Native Glass Card Modifier
            ...
            .background {
                GlassEffectContainer(spacing: enableMorph ? 8 : nil) {
                    RoundedRectangle(cornerRadius: 16, style: .continuous)
                        ...
                        .opacity(0.85 * intensity) // Proper transparency
                        .overlay {
                            if let tint = tint {
                                RoundedRectangle(cornerRadius: 16, style: .continuous)
...
extension View {
    func enhancedGlassCard(
        ...
        modifier(AppleGlassCardModifier(
            variant: variant,
            intensity: intensity,
...
struct LiquidMetricDisplay: View {
    ...
    enum MetricStyle {
        ...
        @MainActor var backgroundGlass: Glass {
            switch self {
            case .standard: return Glass().intensity(0.6)
            case .prominent: return Glass().intensity(0.9).morphStyle(.moderate)
            case .minimal: return Glass().intensity(0.3)
            }
        }
    }
    ...
    var body: some View {
        ...
        .padding(.vertical, 8)
        .glassEffect(
            style.backgroundGlass.tint(iconColor),
...
struct LiquidStatCard: View {
    ...
    enum CardVariant {
        ...
        @MainActor var glass: Glass {
            switch self {
            case .flat: return Glass().intensity(0.2)
            case .glass: return Glass().intensity(0.6).morphStyle(.subtle)
            case .prominent: return Glass().intensity(0.9).morphStyle(.moderate)
            }
        }
    }
